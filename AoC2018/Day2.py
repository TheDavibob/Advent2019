...
def exactly_n(input_string, n):
    return len([item for item, count in collections.Counter(input_string).items() if count == n]) > 0

input_string = '''uqcipadzntnheslgvjjozmkfyr
...
print('Checksum: {} x {} = {}'.format(num_2, num_3, num_2*num_3))

## Second part of day

line_letters = [[letter for letter in line] for line in lines]

def compare_two_lines(line1, line2):
    return len(line_letters[line1]) - sum([a == b for a, b in zip(line_letters[line1], line_letters[line2])])

for line1 in range(len(line_letters)):
    for line2 in range(line1):
        if compare_two_lines(line1, line2) == 1:
            line1_to_use = line1
            line2_to_use = line2
            break

print(lines[line1_to_use])
print(lines[line2_to_use])